...
class _IntroductionState extends State<Introduction>
  ...
  static final _defaultPages = [
    IntroductionPage(
        ...
        text:
            'De appshell is door iconica gebouwd ter ondersteuning van vlotte development van apps!',
    ...
    IntroductionPage(
        ...
        text:
            'Door het grote aantal aan opties en optimaal gebruik van het flutter thema kan de appshell met weinig moeite op verschillende maatwerk situaties worden toegepast',
    ...
    IntroductionPage(
        title: 'Configureren!',
        text:
            'Dit zijn de standaard introductieschermen van de appshell. Geef zelf introductieschermen mee voor optimale aanpassing aan je maatwerk app!',
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      List<IntroductionPage>? confPages = widget.pages;
      setState(() {
        ...
        widget.introductionSettings.buttonMode = widget.introductionSettings.buttonMode;
        widget.introductionSettings.showSkipButton = widget.introductionSettings.showSkipButton ?? true;
        widget.introductionSettings.showFinishButton = widget.introductionSettings.showFinishButton ?? true;
        widget.introductionSettings.layoutStyle = widget.introductionSettings.layoutStyle;
        textAlign = [].contains(widget.introductionSettings.layoutStyle) ? TextAlign.center : TextAlign.left;
      });
    });
  }
  ...
  void _handleTapUp(TapUpDetails details) {
    if (widget.introductionSettings.introductionTapEnabled == true) {
      if (details.globalPosition.dx / MediaQuery.of(context).size.width > 0.5) {
        if (_controller.page!.toInt() < pages.length - 1) {
          _controller.animateToPage(_controller.page!.floor().toInt() + 1,
  ...
  GestureDetector _buildIntroductionPage(
      BuildContext context, IntroductionPage page) {
      ...
      child: Material(
        ...
        child: Stack(
          children: [
            ...
            Column(
              crossAxisAlignment: widget.introductionSettings.introductionTextAlign.toCrossAxisAlignment(),
              children: <Widget>[
  ...
  Widget _buildCenterImage(IntroductionPage page, BuildContext context) {
    if (page.animation != null) {
      ...
      String package = '';
      String image = '';
      List<String> images = page.image!.split(';');
      if (images.length > 1) {
        image = images[0];
        package = images[1];
        return Padding(
  ...
  Widget _buildPageIndicator(BuildContext context) {
    ...
    if (widget.introductionSettings.introductionIndicatorMode == IndicatorMode.Dot) {
...
class DashIndicator extends AnimatedWidget {
  ...
  @override
  Widget build(BuildContext context) {
    int index = (controller.page ?? controller.initialPage).round();
    return Row(
...
// wrapper class to enable old API from shadow Icon with new DecoratedIcon package
class ShadowIcon extends StatelessWidget {

  const ShadowIcon({super.key, required this.icon, required this.shadow});
  final Icon icon;
  final Color shadow;
